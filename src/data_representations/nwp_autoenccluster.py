...
def nwp_autoencoder_fit(hparams, energy_time_series, weather_time_series):
    ...
    autoencoder = model(hparams, weather_time_series[0].values)

    train_idx = weather_time_series.time < pd.to_datetime(hparams.train_split)

    train = weather_time_series.loc[train_idx].values
    train_dl = prepare_dateloader(hparams, train)
    val = weather_time_series.loc[~train_idx].values
    ...
    autoencoder = model.load_from_checkpoint(
        ...
        hparams=hparams, example_input=weather_time_series[0].values
    )
...
def nwp_autoencoder(hparams, energy_time_series, weather_time_series):
    global autoencoder, data_stats
    with torch.no_grad():
        input_data = weather_time_series.values
        input_data = input_data.reshape(-1, 1, *input_data.shape[1:])
        data_tensor = autoencoder.encode(torch.Tensor(input_data))
    data_tensor = data_tensor.cpu()
    return numpy_to_xarray(data_tensor.numpy(), weather_time_series.time)
